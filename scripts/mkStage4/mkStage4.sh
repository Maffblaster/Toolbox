...
# Backup script for Gentoo-based Linux distributions.
#
...
# Date: 2009-04-02
#
# Copied into Digital Survival's GitHub account on 2014-08-14
# Updated the same day by Matthew Marchese
...
# Split command; Makes it possible to fit on a 700MB CD.
...
# Files/devices/folders, which need to be backed up (to preserve folder structure).
...
# Print out the version
 echo -e "\nmkStage4 backup script $version"
 echo -e "=================="
 
# How do you want to backup?
echo -e "\nSelect compression type and backup scope: 
(Use CONTROL-C to abort)\n

Good compression (tar.gz) - Light resources:
 (1) Minimal backup
 (2) Interactive backup
 
Better compression (tar.bz2) - Medium resources:
 (3) Minimal backup
 (4) Interactive backup
 
Best compression (tar.xz) - Heavy resources:
 (5) Minimal backup
 (6) Interactive backup\n"
 
while [ "$option" != '1' -a "$option" != '2' -a "$option" != '3' -a "$option" != '4' -a "$option" != '5' -a "$option" != '6' ]; do
...
# Everything is set, are you sure you want to continue?
echo -ne "\nEverything is set, do you want to continue? (y/n): "
read answer
while [ "$answer" != 'y' ] && [ "$answer" != 'n' ]; do
        echo -ne "Your last input doesn't work. Enter either y or n: "
        read answer
done
